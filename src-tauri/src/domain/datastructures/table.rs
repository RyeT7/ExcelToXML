...
impl Table {
    ...
    pub fn push_keys(&mut self, keys: &Vec<String>) -> Result<(), String> {
        for key in keys {
            match self.data.entry(key.to_string()) {
                Entry::Occupied(o) => {
                    return Err(format!("Duplicate key found: {}", o.key()));
                }
                Entry::Vacant(v) => {
                    v.insert(Vec::new());
                }
    ...
    pub fn rename_key(&mut self, key_mapper: HashMap<String, String>) -> Result<(), String> {
        for new_key in key_mapper.values() {
            if self.data.contains_key(new_key) {
                return Err(format!("Duplicate key: {}", new_key));
            }
        }
    ...
    pub fn group_by(&self, keys: &[String]) -> Result<Vec<Table>, String> {
        ...
        for (k, v) in &self.data {
            if v.len() != len {
                return Err(format!("Column '{}' has inconsistent length", k));
            }
        }
        ...
        for i in 0..len {
            let group_key: Vec<&str> = keys.iter().map(|k| self.data[k][i].as_str()).collect();
    ...
    pub fn get_first(&self, key: &str) -> Result<&str, String> {
        match self.data.get(key) {
            Some(v) => match v.first() {
                Some(vf) => Ok(vf),
                None => Err(format!("No item found in: {}", key)),
            },
            None => Err(format!("Key not found: {}", key)),
        }
    }
